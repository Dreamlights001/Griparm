...
下面这张表对应当前项目实际使用的 `env_layout_tuned.xml`。其中“XML 物理限位”来自 `<joint range=...>`，“项目控制限位”来自位置执行器 `ctrlrange` 和脚本中的目标值约束。

| 电机 / 关节 | 类型 | URDF / MJCF 轴向 | XML 物理限位 (rad) | XML 物理限位 (deg) | 项目控制限位 | 备注 |
|-------------|------|------------------|--------------------|--------------------|--------------|------|
| J1 `J_jianbu` | continuous | `[0, 0, 1]` | 无 | 无 | `[-3.1416, 3.1416] rad` / `[-180.00, 180.00] deg` | 连续关节，项目中由执行器控制范围限制 |
| J2 `J_dabi` | revolute | `[0, 0, 1]` | `[-1.7453, 1.7453]` | `[-100.00, 100.00]` | `[-1.7453, 1.7453] rad` / `[-100.00, 100.00] deg` | XML 中 `actuatorfrcrange=-40 40` |
| J3 `J_Upper` | revolute | `[0, 0, 1]` | `[-1.0472, 1.5708]` | `[-60.00, 90.00]` | `[-1.0472, 1.5708] rad` / `[-60.00, 90.00] deg` | XML 中 `actuatorfrcrange=-25 25` |
| J4 `J_Lower` | continuous | `[0, 0, -1]` | 无 | 无 | `[-3.1416, 3.1416] rad` / `[-180.00, 180.00] deg` | 连续关节，轴向为负，控制效果相对直觉会反向 |
| J5 `J_wrist` | revolute | `[0, 0, 1]` | `[-1.5708, 1.5708]` | `[-90.00, 90.00]` | `[-1.5708, 1.5708] rad` / `[-90.00, 90.00] deg` | 腕部旋转限位 |
| J6 `J_hand` | continuous | `[0, 0, -1]` | 无 | 无 | `[-3.1416, 3.1416] rad` / `[-180.00, 180.00] deg` | 连续关节，轴向为负，控制效果相对直觉会反向 |
| J7 `Claw_left` / `claw_right` | prismatic | left `[-1, 0, 0]`, right `[1, 0, 0]` | left `[0, 0.04] m`, right `[-0.04, 0] m` | 不适用 | 采集默认 `0.0 -> 0.038 m`，XML 最大 `0.0 -> 0.04 m` | 左爪主动平移，右爪镜像平移 |

J7 夹爪开合范围换算：

| 项目 | 单个爪片行程 | 两爪相对开合变化量 |
|------|--------------|--------------------|
| XML 最大范围 | `0.04 m` / `40 mm` | `0.08 m` / `80 mm` |
| 采集默认闭合目标 | `0.038 m` / `38 mm` | `0.076 m` / `76 mm` |

说明：

- J7 是平动关节，不存在角度限位；如果映射到齿轮齿条电机角度，需要知道小齿轮节圆半径或传动比。
- 当前 readme 上方的夹爪 `rpm` 使用 `r = 10 mm` 的等效假设，只用于速度换算，不代表模型中显式存在该齿轮。
